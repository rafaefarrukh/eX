...
## import
courses <- rbind(
    read_xls("dataSemesterSchedule/2024SU.xls"),
    read_xls("dataSemesterSchedule/2024FA.xls"),
    read_xls("dataSemesterSchedule/2025SP.xls")
    )
## rename
...
server <- function(input, output) {
    ...
    timedata <- reactive({
        ...
        #temp1 <- data.frame(day = 1, rbind(dum, filter(selected(), str_detect(days, "M"))[,c(3:11)]))
        #temp2 <- data.frame(day = 2, rbind(dum, filter(selected(), str_detect(days, "T"))[,c(3:11)]))
        #temp3 <- data.frame(day = 3, rbind(dum, filter(selected(), str_detect(days, "W"))[,c(3:11)]))
        #temp4 <- data.frame(day = 4, rbind(dum, filter(selected(), str_detect(days, "R"))[,c(3:11)]))
        #temp5 <- data.frame(day = 5, rbind(dum, filter(selected(), str_detect(days, "F"))[,c(3:11)]))
        
        #temp <- data.frame(rbind(temp1, temp2, temp3, temp4, temp5))
